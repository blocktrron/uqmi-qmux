...
static struct {
	...
	QmiWdaDataAggregationProtocol aggregation_protocol;
} wda_aggregation_info = {
	.max_size = 0,
	.max_datagrams = 0,
	.aggregation_protocol = QMI_WDA_DATA_AGGREGATION_PROTOCOL_DISABLED,
};
...
cmd_wda_set_data_format_send(struct qmi_msg *msg, QmiWdaLinkLayerProtocol link_layer_proto)
{
	struct qmi_wda_set_data_format_request data_req = {
		...
		QMI_INIT(uplink_data_aggregation_protocol, wda_aggregation_info.aggregation_protocol),
		QMI_INIT(downlink_data_aggregation_protocol, wda_aggregation_info.aggregation_protocol),
		QMI_INIT(downlink_data_aggregation_max_datagrams, wda_aggregation_info.max_datagrams),
		QMI_INIT(downlink_data_aggregation_max_size, wda_aggregation_info.max_size),
	};
...
#define cmd_wda_data_aggregation_protocol_cb no_cb

static enum qmi_cmd_result cmd_wda_data_aggregation_protocol_prepare(
	...
	char *arg)
{
	for (uint32_t i = 0; i < ARRAY_SIZE(aggregation_protocols); i++) {
		...
		wda_aggregation_info.aggregation_protocol = aggregation_protocols[i].aggreg;
		return QMI_CMD_DONE;
	}

	uqmi_add_error("Invalid protocol (valid: tlp, qc-cm, mbim, rndis, qmap");
	return QMI_CMD_EXIT;
}
	...
	char *arg)
{
	...
	wda_aggregation_info.max_datagrams = max_datagrams;
	return QMI_CMD_DONE;
}
	...
	char *arg)
{
	...
	wda_aggregation_info.max_size = max_size;
	return QMI_CMD_DONE;
}
